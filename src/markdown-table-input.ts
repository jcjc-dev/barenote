 ...
 */
export function parseTableSeparatorLine(
  text: string,
    ...
    appendTransaction: (transactions, _oldState, newState) => {
      ...
      for (let i = replacements.length - 1; i >= 0; i--) {
        ...
        const headerCells = headers.map((text, colIdx) => {
          ...
          return tableHeaderSchema.type(ctx).create(
            { alignment: alignments[colIdx] || "left" },
            para,
          );
        });
        ...
        const dataCells = headers.map((_text, colIdx) => {
          const para = schema.nodes.paragraph.create();
          return tableCellSchema.type(ctx).create(
            { alignment: alignments[colIdx] || "left" },
            para,
          );
        });
      ...
      try {
        tr = tr.setSelection(
          TextSelection.near(tr.doc.resolve(replacements[0].from + 1)),
        );
      } catch {
